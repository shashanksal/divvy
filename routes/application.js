...
router.get('/common-dest',function (req,res,next) {
    let result = Trips.aggregate([{
...
router.post('/common-dest',function (req,res,next) {
    let from_station = (req.body.from_station_name);

    let resultArr = [];

    let common_dest = Trips.aggregate([
        ...
        resultArr.push(`Common Destination from - From Station =  ${docs[0]._id}` );
    });

    let age_grp = Trips.aggregate([
        {"$match" : { "from_station_name": { $eq :  from_station } }}
    ],(err,docs)=>{
        let bucket = [0,0,0,0];
        docs.forEach(doc=>{
...
router.get('/top-revenue-gen',function (req,res,next) {
    Trips.aggregate(
        [
            {"$group" :
                    {_id:"$from_station_name",
            ...
            res.status(200).send(`The Top 3 revenue generating stations are : ${JSON.stringify(station)}`);
        })
});

router.get('/bike-repair', (req,res,next)=>{

    Trips.aggregate(
        [
            {"$group" : {_id:"$bikeid", count:{$sum:1} , duration:{ $sum: { $multiply: [ "$tripduration", 1/60 , 1/1000 ] } },
            ...
            let bike_ids = [];
            docs.forEach(doc=>{
                if(doc.duration.toFixed(2)>1.00 ){
                    bike_ids.push(doc._id);
                }

            })
            res.status(200).send(`The Bike Ids that need repairs are : ${JSON.stringify(bike_ids)}`);
        })
